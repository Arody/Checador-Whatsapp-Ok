                   ...
                   summary.sessions.map((session, sIdx) => {
                      ...
                      return (
                        <tr key={`${summary.userId}-${summary.date}-${sIdx}`} style={{ borderBottom: isFirst && rowSpan > 1 ? 'none' : '1px solid var(--border)' }}>
                          ...
                          <td>{session.checkIn ? new Date(session.checkIn.timestamp).toLocaleTimeString('es-MX', {hour: '2-digit', minute:'2-digit'}) : '-'}</td>
                          <td>{session.checkOut ? new Date(session.checkOut.timestamp).toLocaleTimeString('es-MX', {hour: '2-digit', minute:'2-digit'}) : '-'}</td>
